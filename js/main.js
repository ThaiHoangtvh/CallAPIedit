...
const date_time = getQuery('#date-time').value;
let ticked = "";
let ticked2 = "";
...
const getForm = async() => {
    ...
    var htmls = getData.map(function(response) {
        return `<tr><td>${response.firstname}</td><td>${response.lastname}</td><td>${response.patientage}</td><td>${response.nickname}</td><td>${response.email}</td><td>${response.gender}</td><td>${response.phone}</td><td>${response.spoucename}</td><td>${response.whome}</td><td>${response.marital}</td><td>${response.occupation}</td><td>${response.retired}</td><td>${response.date}</td></tr>`
    })
